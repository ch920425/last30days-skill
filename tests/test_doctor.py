...
class UnconfiguredXWithBrokenNode(unittest.TestCase):
    """No bearer token keeps X unconfigured regardless of unrelated binaries."""

    def test_x_rolls_up_off_with_bearer_prescription(self):
        with _Hermetic(probe_map={"node": health.BROKEN}), \
             mock.patch("lib.backends.which", lambda name: "/usr/local/bin/xurl" if name == "xurl" else None):
            report = doctor.build_report({})
        record = report["sources"]["x"]
        self.assertEqual("off", record["tier"])
        self.assertEqual("unconfigured", record["status"])
        xurl = next(b for b in record["backends"] if b["name"] == "xurl")
        self.assertEqual("missing", xurl["status"])
        self.assertIn("X_BEARER_TOKEN", xurl["fix"])
        self.assertNotIn("node", xurl["fix"].lower())
